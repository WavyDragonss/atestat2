...
function parseForLineCsvStep(line: string) {
  const normalized = normalizeForMatch(line);
  const match = normalized.match(
    /^pentru\s+([a-z_][a-z0-9_]*)\s*(?:<-|=)\s*(.+?)\s*,\s*(.+?)\s*,\s*(.+?)(?:\s+executa)?$/i,
  );
  ...
  const start = normalizeOperators(match[2]);
  const end = normalizeOperators(match[3]);
  const step = normalizeOperators(match[4]);
...
export function compilePseudocode(input: string, options: CompileOptions = {}): CompileResult {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    const parsedForCsv = parseForLineCsvStep(trimmed);
    const bestFor = parsedForCsv ?? parsedFor;
    if (bestFor) {
      const { variable, start, end, step } = bestFor;
    ...
    const swap = parseSwap(trimmed);
    if (swap) {
      const { left, right } = swap;
      if (!declaredVars.has(left)) {
        generated.push(`${currentIndent(indentLevel)}let ${left} = 0;`);
        declaredVars.add(left);
      }
      if (!declaredVars.has(right)) {
        generated.push(`${currentIndent(indentLevel)}let ${right} = 0;`);
        declaredVars.add(right);
      }
      generated.push(`${currentIndent(indentLevel)}[${left}, ${right}] = [${right}, ${left}];`);
      continue;
    }
